  ...
  if (info.Length() > 6 && !info[6]->IsUndefined()) {
    ...
    if (v8Headers->Length() >= 1) {
      for (unsigned int i = 0; i < v8Headers->Length(); i++) {
        v8::Local<v8::Object> header = v8Headers->Get(i)->ToObject(Nan::GetCurrentContext()).ToLocalChecked();
        if (header.IsEmpty()) {
          continue;
        }
        v8::Local<v8::Array> props = header->GetOwnPropertyNames(Nan::GetCurrentContext()).ToLocalChecked();
